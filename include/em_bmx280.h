...
public:
    ...
    const char* getDeviceName() const {
        ...
        return isBme280() ? "BME280" : "BMP280";
    }

    bool begin(uint8_t addr, IirFilter filter = IirFilter::OFF);
...
private:
    ...
    bool read_register(uint8_t reg, uint8_t* buffer, size_t length) {
        return m_i2c.write_read(m_addr, &reg, 1, buffer, length);
    }
...
inline bool EmBmx280::begin(uint8_t addr, IirFilter filter) {
...
inline bool EmBmx280::read(float& temperature, float& pressure, float* humidity) {
